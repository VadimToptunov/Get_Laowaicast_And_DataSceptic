...
def grab_podcast_urls():
    ...
    for pagenum in range(25, 1, -1):
        url = f"https://laowaicast.ru/category/episode/page/{pagenum}"
        print(f"Page #{pagenum}")
        page = requests.get(url)
        data = page.text
        soup = BeautifulSoup(data, "html.parser")
        laowaicast_links = soup.find_all("a", attrs={'href': re.compile("^(http|https)://media.*"), "title": "Скачать"})
    ...
    for url_pod in pod_links:
        download_podcasts(url_pod)


def download_podcasts(url_pod):
    r = requests.get(url_pod, allow_redirects=True)
    dirname = r".\laowaicast_podcasts"
